...
// *** NEW PACKET_SIZE: 1 byte per sample (1800 bytes total for samples) ***
const PACKET_SIZE = 1 + 6 + 1 * NUM_SAMPLES + 1; // Expected total size: 1808 bytes
...
try {
    ...
    port.on('data', (data) => {
        ...
        while (buffer.length >= PACKET_SIZE) {
            ...
            // 2. Unpack Samples (1800 bytes)
            const samplesBuffer = payload.slice(6);
            const samples = [];
            for (let i = 0; i < NUM_SAMPLES; i++) {
                // Read 8-bit (1-byte) sample data
                samples.push(samplesBuffer.readUInt8(i)); 
            }
