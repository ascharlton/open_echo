...
function startComPortListener() {
    ...
    port.on('data', sonarPacketHandler); 
    //
    // ----------------------------------------------------------------------
}
 ...
 */
function sonarPacketHandler(data) {
    ...
    while (comBuffer.length >= PACKET_SIZE) {
        ...
        //console.log("[WARN] calculated checksum: ", calculatedChecksum);
...
async function processDataPacket(rawSamples) {
    ...
    if (!gps.lat || !gps.lon) {
        console.log("[LOG WARN] Skipping log due to uninitialized GPS data.");
        return;
    }
