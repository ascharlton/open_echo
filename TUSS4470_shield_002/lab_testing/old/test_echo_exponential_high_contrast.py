...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        n_low = max(1, int(len(spectrogram) * 0.20))  # number of samples in lowest 10%
        ...
        if CONTRAST_ENHANCE:
            # Contrast enhancement based on global stats, not per-frame normalization
            sigma = np.std(self.ema_data)
            mean = np.mean(self.ema_data)
