...
class SettingsDialog(QWidget):
    def __init__(self, parent=None, current_gradient='spectrum', current_speed=330, nmea_enabled=False, nmea_port=10110, nmea_address="127.0.0.1"):
...
class WaterfallApp(QMainWindow):
    def __init__(self):
        ...
        self.current_gradient = 'spectrum'  # default color scheme
        ...
        self.setGeometry(0, 0, 1280, 800)  # Portrait mode for Raspberry Pi screen
        ...
        # === Inline Contrast Recovery Button BELOW Plot ===
        self.reset_contrast_button = QPushButton("Reset Contrast")
        self.reset_contrast_button.setFixedHeight(28)
        self.reset_contrast_button.setStyleSheet("""
            QPushButton {
        ...
        main_layout.addWidget(self.reset_contrast_button)
        ...
        self.colorbar.item.gradient.loadPreset('spectrum')
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        # --- Replace first 144 points with random values from the lowest 10% of the slice ---
        n_low = max(1, int(len(spectrogram) * 0.10))  # number of samples in lowest 10%
        lowest_values = np.sort(spectrogram)[:n_low]

        # Randomly select 144 values (with replacement) from the lowest 10% for the dead zone
        spectrogram[:150] = np.random.choice(lowest_values, size=150, replace=True)

        # --- Insert modified slice ---
        self.data[-1, :] = spectrogram
        
        # --- Initialize or update exponential moving average (EMA) ---
        alpha = 0.3  # smoothing factor, tweak 0.1–0.5 as needed

        if not hasattr(self, "ema_data"):
            # Initialize EMA on first run
            self.ema_data = self.data.copy().astype(float)
        else:
            # Update EMA: new = α*current + (1-α)*previous
            self.ema_data = alpha * self.data + (1 - alpha) * self.ema_data

        # --- Emphasize expected range (10–150) ---
        in_range_mask = (self.ema_data[-1, :] >= 10) & (self.ema_data[-1, :] <= 150)

        # Create a copy for visualization
        emphasized_slice = self.ema_data[-1, :].copy()
        emphasized_slice[~in_range_mask] *= 0.1  # reduce non-relevant values by 50%
        self.ema_data[-1, :] = emphasized_slice

        # --- Initialize or update global contrast tracking ---
        contrast_alpha = 0.05  # 0.05 slower EMA for global brightness
        current_mean = np.mean(self.ema_data)
        current_sigma = np.std(self.ema_data)
        # Optional: cap extremes to prevent sudden bursts from skewing global stats
        #current_sigma = np.clip(current_sigma, 0.5 * self.global_sigma, 2.0 * self.global_sigma)

        if not hasattr(self, "global_mean"):
            self.global_mean = current_mean
            self.global_sigma = current_sigma
        else:
            # Update running average of mean/std to stabilize levels
            self.global_mean = (1 - contrast_alpha) * self.global_mean + contrast_alpha * current_mean
            self.global_sigma = (1 - contrast_alpha) * self.global_sigma + contrast_alpha * current_sigma

        # --- Display processed EMA data ---
        self.imageitem.setImage(self.ema_data.T, autoLevels=False)

        # --- Use stable global levels for brightness ---
        self.imageitem.setLevels(
            (self.global_mean - 2 * self.global_sigma,
            self.global_mean + 2 * self.global_sigma)
        )
        ...
        if CONTRAST_ENHANCE:
            # Adaptive brightness
            mean = np.mean(self.ema_data[-5:, :])  # last 50 slices only
            sigma = np.std(self.ema_data[-5:, :])
            self.imageitem.setImage(self.ema_data.T, autoLevels=False)
    ...
    def reset_contrast(self):
        """Recalculate and reset the global contrast statistics for stable brightness."""
        if hasattr(self, "ema_data"):
            self.global_mean = np.mean(self.ema_data)
            self.global_sigma = np.std(self.ema_data)
            print("✅ Contrast Reset: global mean/sigma recalibrated.")
            # Immediately reapply the new brightness levels
            self.imageitem.setLevels(
                (self.global_mean - 2 * self.global_sigma,
                self.global_mean + 2 * self.global_sigma)
            )
        else:
            print("⚠️ No EMA data yet; nothing to reset.")
