...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        n_low = max(1, int(len(spectrogram) * 0.8))  # number of samples in lowest 10%
        lowest_values = np.sort(spectrogram)[:n_low]
        spectrogram[:144] = np.random.choice(lowest_values, size=144, replace=True)
        ...
        # --- NEW: Temporal Stability Filter ---
        window_size = 8  # how many past frames to consider for stability
        if self.data.shape[0] >= window_size:
            recent_window = self.data[-window_size:, :]  # last N slices
            mean_recent = np.mean(recent_window, axis=0)
            std_recent = np.std(recent_window, axis=0)

            # Compute stability score: low std means consistent (likely real reflection)
            # We invert std so that stable regions get higher scores
            stability_score = 1 / (1 + std_recent)  # avoids divide-by-zero
            stability_score /= np.max(stability_score)  # normalize to [0, 1]

            # Create adaptive suppression factor: more stable → less suppression
            suppression_factor = 0.2 + 0.8 * stability_score  # between 0.2–1.0

            # Apply suppression adaptively across depth
            current_slice = self.ema_data[-1, :]
            mean_plus_std = mean_recent + std_recent

            # Points below the reflection threshold are dimmed by suppression factor
            below_threshold = current_slice < mean_plus_std
            current_slice[below_threshold] *= suppression_factor[below_threshold]

            self.ema_data[-1, :] = current_slice

        # --- Optional Contrast Enhancement ---
        CONTRAST_ENHANCE = True

        if CONTRAST_ENHANCE:
            img = self.ema_data.copy()
            min_val, max_val = np.min(img), np.max(img)
            if max_val > min_val:
                img = (img - min_val) / (max_val - min_val) * 255.0
            else:
                img[:] = 0
            self.imageitem.setImage(img.T, autoLevels=False)
        else:
            self.imageitem.setImage(self.ema_data.T, autoLevels=False)

        sigma = np.std(self.data)
        mean = np.mean(self.data)
        self.imageitem.setLevels((mean - 5 * sigma, mean + 5 * sigma))
