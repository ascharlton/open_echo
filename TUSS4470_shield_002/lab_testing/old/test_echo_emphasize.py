...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        # Only process if we have at least 2 slices filled
        if self.data.shape[0] >= 2:
            last = self.data[-1, :]
            prev = self.data[-5, :]

            # Find indices where data points are close (within ±5)
            consistent_mask = np.abs(last - prev) <= 5

            # Focus only on expected reflection range (10–150)
            in_range_mask = (last >= 10) & (last <= 150)
            emphasize_mask = consistent_mask & in_range_mask

            # Reduce all other values by 50%
            reduced = self.data[-1, :] * 0.5
            self.data[-1, :] = np.where(emphasize_mask, last, reduced)
