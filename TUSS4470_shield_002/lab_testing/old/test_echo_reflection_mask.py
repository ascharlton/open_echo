...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        n_low = max(1, int(len(spectrogram) * 0.50))  # number of samples in lowest 10%
        lowest_values = np.sort(spectrogram)[:n_low]

        # Randomly select 144 values (with replacement) from the lowest 10%
        ...
        alpha = .3  # smoothing factor, tweak 0.1–0.5 as needed
        ...
        if self.data.shape[0] >= window_size:
            ...
            # The "prominent" reflection points = those consistently above mean + 1σ
            reflection_mask = self.ema_data[-1, :] > (mean_recent + 0.5*std_recent)

            # Everything else (unstable noise blobs) gets attenuated
            suppressed_slice = self.ema_data[-1, :].copy()
            suppressed_slice[~reflection_mask] *= 0.1  # reduce by 70%
            self.ema_data[-1, :] = suppressed_slice
