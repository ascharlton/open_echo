...
class WaterfallApp(QMainWindow):
    ...
    def waterfall_plot_callback(self, spectrogram, depth_index, temperature, drive_voltage):
        ...
        # --- Noise suppression based on temporal consistency ---

        if self.data.shape[0] >= 2:
            current = self.ema_data[-1, :]
            previous = self.ema_data[-2, :]

            # Allow for reflection jitter within a few indices
            kernel_size = 5  # how far left/right to check for consistency
            consistent_mask = np.zeros_like(current, dtype=bool)

            for offset in range(-kernel_size, kernel_size + 1):
                shifted = np.roll(previous, offset)
                consistent_mask |= np.abs(current - shifted) < 5  # value similarity check

            # Suppress everything that isn't consistent with the previous slice
            filtered_slice = current.copy()
            filtered_slice[~consistent_mask] *= 0.2  # 80% reduction for non-persistent data
            self.ema_data[-1, :] = filtered_slice
